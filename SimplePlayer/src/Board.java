...
public class Board {
	...
	public static boolean isAdjacent(Cell previous, Cell current){
        ...
        if(current.x == previous.x && current.y == previous.y){
        	return false;
                ...
                (current.x - previous.x <= 1) && (current.x - previous.x >= 0)
                &&
                (current.y - previous.y <= 1) && (current.y - previous.y >= 0)
        ){
        	return true;
        } else if(
                (current.x - previous.x <= 0) && (current.x - previous.x >= -1)
                &&
                (current.y - previous.y <= 0) && (current.y - previous.y >= -1)
        ){
        	return true;
        } else if((current.x - previous.x <= 0) && (current.x - previous.x >= -1)
                        &&
                        (current.y - previous.y <= 1) && (current.y - previous.y >= 0)
        ){
        	return true;
        } else if((current.x - previous.x <= 1) && (current.x - previous.x >= 0)
                        &&
                        (current.y - previous.y <= 0) && (current.y - previous.y >= -1)
        ){
        	return true;      
	...
	public void generateEdgeNodes(int n){
		...
		// Populate top right range
		for(i=1;i<n-1;i++){ 
			String temp = i+","+j;
			top_r.add(temp);
			j++;
		}
		// Populate bottom left range
		for(i=n;i<2*n-2;i++){
			String temp = i+",0";
			bot_l.add(temp);
		}
	 ...
	 * and recursive traversing through the connecting neighbour
	 * nodes.
	 */
	public boolean checkTripod(NewGraph g){
		...
		for (Cell c: g.rootList){
			...
			// Traverse through the neighbours and get their board positions
			for (i=0;i<c.neighborList.size();i++){   
				temp = c.neighborList.get(i).x+","+c.neighborList.get(i).y;
				checkedNodes.push(temp);
	...
	public static boolean isUnchecked(Cell current, LinkedList<Cell> cellExclusionList, int cellExclusionListSize){
		...
		while(i < cellExclusionListSize){
			if((cellExclusionList.get(i).x == current.x) && (cellExclusionList.get(i).y == current.y)){
